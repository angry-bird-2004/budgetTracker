...
import { removeIncomeEnvelope } from '@/src/services/mutations';
import { useBudgetStore } from '@/src/store/budget';
import { usePreferences } from '@/src/store/preferences';
import { formatDisplayDate } from '@/src/utils/dates';
import { inPeriod } from '@/src/utils/period';

export default function IncomeEnvelopeDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const router = useRouter();
  const { format } = useMoney();
  const period = usePreferences((state) => state.period);
  const env = useBudgetStore((state) => state.incomeEnvelopes.find((item) => item.clientId === id));
  const txs = useBudgetStore((state) =>
    state.transactions.filter((tx) => tx.incomeClientId === id && inPeriod(tx.date, period)),
  );

  if (!env) {
    return (
      <Screen>
        <EmptyState icon="alert-circle-outline" title="Not found" body="This income envelope is no longer on this device." />
      </Screen>
    );
  }

  const onDelete = () => {
    Alert.alert('Delete income envelope', 'This envelope will be removed and synced later.', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete',
        style: 'destructive',
        onPress: async () => {
          await removeIncomeEnvelope(env.clientId);
          router.back();
        },
      },
    ]);
    ...
    <Screen>
      <ScrollView contentContainerClassName="pb-10">
        ...
        <Card className="mt-4">
          ...
          <Text className="text-sm text-slate-400">In / Out</Text>
          <Text className="mb-3 text-white">
            {format(env.income)} in · {format(env.consumed)} out
          </Text>
          <Text className="text-sm text-slate-400">Tax</Text>
          <Text className="mb-3 text-amber-400">{format(env.tax)}</Text>
        ...
        <View className="mt-4 gap-3">
          <Button
            title="Edit"
            onPress={() =>
              router.push({ pathname: '/(app)/income-envelope/form', params: { id: env.clientId } })
            }
          />
