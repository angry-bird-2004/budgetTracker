...
import { saveIncomeEnvelope } from '@/src/services/mutations';
import { useBudgetStore } from '@/src/store/budget';

export default function IncomeEnvelopeFormScreen() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id?: string }>();
  const { toBase, fromBase } = useMoney();
  const incomeEnvelopes = useBudgetStore((state) => state.incomeEnvelopes);
  const existing = useMemo(
    () => incomeEnvelopes.find((item) => item.clientId === id),
    [incomeEnvelopes, id],
  );
  ...
  const onSave = async () => {
    ...
    if (!Number.isFinite(parsed) || parsed < 0) return setError('Allocated amount must be 0 or more');
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to save income envelope');
    } finally {
      setSaving(false);
    }
  };
    ...
    <Screen>
      <ScrollView>
        <Text className="mb-4 text-lg font-semibold text-white">
          {existing ? 'Edit income envelope' : 'New income envelope'}
        </Text>
